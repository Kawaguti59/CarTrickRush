...
- **UI は Presenter / View 構成で管理**  
  ...
  必要な Manager などは VContainer 経由で受け取る

- **横断的な処理は Manager に集約**  
  スコア・シーン遷移・入力・音声・セーブなど、いろんな場所から使う処理は Manager にまとめている。  
  VContainer を使って Manager を生成し、各シーンのスクリプトへ参照を渡す

- **シーン分割 + Additive 読み込みによる UI 管理**  
  ポーズ画面や設定画面は追加読み込みで管理し、ゲーム本体と独立した UI 制御を実現

用途ごとに設計パターンを使い分けることで、短期開発でも保守性と拡張性を両立する構成を目指しました。

### ライブラリ（UniTask / R3 / VContainer）

あわせて以下のライブラリを導入しています。

| ライブラリ | なんとなくの役割 | だいたいこんなところで使っている |
|------------|------------------|----------------------------------|
| **UniTask** | 待ち時間の処理を書きやすくする | ゴール演出、設定画面を閉じるとき、シーン遷移のフェード |
| **R3** | 値や入力が変わったタイミングで処理を走らせる | スコア表示の更新、回転・ポーズ入力 |
| **VContainer** | 必要な Manager などを自動で渡してくれる | Manager の生成と、各スクリプトへの参照渡し |
